...
> {
         ...
         if (this.props.webPartTitleProps.displayMode === DisplayMode.Edit) {
            renderWpContent = (
               <MessageBar messageBarType={MessageBarType.info}>
                  {webPartStrings.General.NoAvailableFilterMessage}
               </MessageBar>
            );
         }
      } else {
         // Content loading
         templateContent = this.props.templateService.getTemplateMarkup(
   ...
   public componentDidMount() {
      ...
      // Initial state
      this.getFiltersToDisplay(
         this.props.availableFilters,
         [],
         this.props.filtersConfiguration
      );
   ...
   public componentDidUpdate(
      prevProps: ISearchFiltersContainerProps,
      prevState: ISearchFiltersContainerState
   ) {
      // When filters configuration is updated or the layout is changed
      if (
         !isEqual(prevProps.selectedLayoutKey, this.props.selectedLayoutKey) ||
         !isEqual(prevProps.properties, this.props.properties)
      ) {
         const updatedfilters = this.resetSelectedFilterValues(
            this.state.currentUiFilters
         );
         const submittedFilters =
            this.getSelectedFiltersFromUIFilters(updatedfilters);
      ...
      if (
         !isEqual(prevProps.availableFilters, this.props.availableFilters) ||
         (!isEqual(prevState.currentUiFilters, this.state.currentUiFilters) &&
            prevState.currentUiFilters.length > 0)
      ) {
         this.getFiltersDeepLink();
         this.getFiltersToDisplay(
            this.props.availableFilters,
         ...
         const submittedFilters = this.getSelectedFiltersFromUIFilters(
            this.state.currentUiFilters
         );
   ...
   private getFiltersToDisplay(
      ...
      const updatedFilters: IDataFilterInternal[] = availableFilters.map(
         (availableFilter) => {
            ...
            if (filterConfiguration) {
               ...
               const selectedFilterIdx = currentUiFilters
                  .map((selectedFilter) => {
                     return selectedFilter.filterName;
                  })
                  .indexOf(availableFilter.filterName);
               ...
               if (availableFilter.values.length === 0) {
                  if (selectedFilterIdx !== -1) {
                     // Set count to 0
                     values = currentUiFilters[selectedFilterIdx].values
                        .map((value) => {
                           // Reset the count for already selected refiners
                           if (
                              ((value.selected || value.selectedOnce) &&
                                 filterConfiguration.isMulti) ||
                              (value.selected && !filterConfiguration.isMulti)
                           ) {
                              value.count = 0;
                           } else {
                              return null;
                  ...
                  values = availableFilter.values.map(
                     (availableValue: IDataFilterResultValue) => {
                        ...
                        if (selectedFilterIdx !== -1) {
                           const valueIdx = currentUiFilters[
                              selectedFilterIdx
                           ].values
                              .map((value) => {
                                 return value.value;
                              })
                              .indexOf(availableValue.value);
                           ...
                           if (valueIdx === -1) {
                              ...
                              const updatedValue =
                                 currentUiFilters[selectedFilterIdx].values[
                                    valueIdx
                                 ];
               ...
               if (selectedFilterIdx !== -1) {
                  const additionalValues = currentUiFilters[
                     selectedFilterIdx
                  ].values.map((value) => {
                  ...
                  values = values.concat(
                     additionalValues.filter((value) => value)
                  );
               }
               ...
               const alreadySubmittedValuesForFilter = flatten(
                  this.state.submittedFilters
                     .filter((s) => s.filterName === availableFilter.filterName)
                     .map((v) => v.values)
               )
                  .map((t) => t.value)
                  .sort();
               ...
               const canApply = !isEqual(
                  currentSelectedValuesInUiForFilter,
                  alreadySubmittedValuesForFilter
               );
               const canClear =
                  alreadySubmittedValuesForFilter.length > 0 ||
                  hasSelectedValues;
               ...
               const filterOperator = currentUiFilters[selectedFilterIdx]
                  ? currentUiFilters[selectedFilterIdx].operator
                  : filterConfiguration.operator;

               // Merge information with filter configuration and other useful proeprties
               const filterResultInternal: IDataFilterInternal = {
                  displayName: filterConfiguration.displayValue
                  ...
                  showCount: !filterConfiguration.showCount
                     ? false
                     : filterConfiguration.showCount,
                  expandByDefault: !filterConfiguration.expandByDefault
                     ? false
                     : filterConfiguration.expandByDefault,
      ...
      this.setState({
         currentUiFilters: update(this.state.currentUiFilters, {
            $set: sortBy(
               updatedFilters.filter((updatedFilter) => updatedFilter),
               "sortIdx"
            ),
         }),
      });
   ...
   private onFilterValuesUpdated(filterInfo: IDataFilterInfo) {
      ...
      const filterConfigIdx = this.props.filtersConfiguration
         .map((filter) => {
            return filter.filterName;
         })
         .indexOf(filterInfo.filterName);

      if (filterConfigIdx !== -1) {
         const filterConfiguration =
            this.props.filtersConfiguration[filterConfigIdx];

         // Get the index of the filter in the current selected filters collection
         const filterIdx = this.state.currentUiFilters
            .map((filter) => {
               return filter.filterName;
            })
            .indexOf(filterInfo.filterName);
         ...
         if (filterIdx !== -1) {
            ...
            if (filterInfo.operator) {
               currentUiFilters = update(currentUiFilters, {
                  [filterIdx]: { operator: { $set: filterInfo.operator } },
               });
            }
            ...
            filterInfo.filterValues.map((filterValue) => {
               ...
               const valueIdx = currentUiFilters[filterIdx].values
                  .map((value) => {
                     return value.value;
                  })
                  .indexOf(filterValue.value);

               if (valueIdx === -1) {
                  // If the value does not exist yet, we add it to the selected values
                  currentUiFilters = update(currentUiFilters, {
                     [filterIdx]: { values: { $push: [filterValueInternal] } },
                  });
               } else {
                  // Otherwise, we update the value in selected values
                  currentUiFilters = update(currentUiFilters, {
                     [filterIdx]: {
                        values: { [valueIdx]: { $set: filterValueInternal } },
                     },
                  });
               }
            });
         } else {
            const filterValuesInternal: IDataFilterValueInternal[] =
               filterInfo.filterValues.map((filterValue) => {
                  return {
                     selected: filterValue.selected,
                     name: filterValue.name,
            ...
            const filterResultInternal: IDataFilterInternal = {
               ...
               hasSelectedValues:
                  filterInfo.filterValues.filter((value) => value.selected)
                     .length > 0,
               selectedOnce: true,
               isMulti: !filterConfiguration.isMulti
                  ? false
               ...
               operator: filterInfo.operator
                  ? filterInfo.operator
                  : currentUiFilters[filterIdx].operator,
               selectedTemplate: filterConfiguration.selectedTemplate,
               sortIdx: filterConfiguration.sortIdx,
            };
            ...
            currentUiFilters = update(this.state.currentUiFilters, {
               $push: [filterResultInternal],
            });
         }

         if (!filterConfiguration.isMulti || filterInfo.forceUpdate) {
            const submittedFilters =
               this.getSelectedFiltersFromUIFilters(currentUiFilters);
    ...
    */
   private getSelectedFiltersFromUIFilters(
      currentUiFilters: IDataFilterInternal[]
   ): IDataFilter[] {
      const selectedFilters: IDataFilter[] = currentUiFilters.map(
         (selectedFilter) => {
            ...
            if (
               selectedFilter.values.some((v) => v.selected) &&
               !selectedFilter.isMulti &&
               selectedFilter.selectedTemplate !==
   ...
   private bindFilterEvents() {
      this.componentRef.current.addEventListener(
         ExtensibilityConstants.EVENT_FILTER_UPDATED,
         ((ev: CustomEvent) => {
   ...
   private bindApplyFiltersEvents() {
      this.componentRef.current.addEventListener(
         ExtensibilityConstants.EVENT_FILTER_APPLY_ALL,
         ((ev: CustomEvent) => {
            ev.stopImmediatePropagation();
   ...
   private bindClearFiltersEvents() {
      this.componentRef.current.addEventListener(
         ExtensibilityConstants.EVENT_FILTER_CLEAR_ALL,
         ((ev: CustomEvent) => {
            ev.stopImmediatePropagation();

            const updatedfilters = this.state.currentUiFilters.map(
               (selectedFilter) => {
            ...
            const updateSubmittedFilters = this.state.submittedFilters.map(
               (submittedFilter) => {
            ...
            const appliedFilters = updateSubmittedFilters.filter(
   ...
   private bindFilterValueOperatorUpdated() {
      this.componentRef.current.addEventListener(
         ExtensibilityConstants.EVENT_FILTER_VALUE_OPERATOR_UPDATED,
         ((ev: CustomEvent) => {
            ev.stopImmediatePropagation();
            ...
            const filters = this.state.currentUiFilters.map((filter) => {
               ...
               const submittedValues = this.state.submittedFilters.filter(
                  (f) =>
                     f.filterName === ev.detail.filterName &&
                     f.values.filter(
                        (v) =>
                           selectedValues
                              .map((s) => s.value)
               ...
               if (filter.filterName === ev.detail.filterName) {
                  ...
                  filter.canApply =
                     ...
                     (filter.canApply && submittedValues.length === 0);
                  filter.operator = ev.detail.operator;
               }
   ...
   private getTemplateContext(): ISearchFiltersTemplateContext {
      return {
         ...
         selectedOnce:
            this.state.currentUiFilters.filter(
               (currentFilter) => currentFilter.selectedOnce
            ).length > 0,
         properties: {
   ...
   private getFiltersDeepLink() {
      const queryString = UrlHelper.getQueryStringParam(
         DEEPLINK_QUERYSTRING_PARAM,
         window.location.href
      );

      if (queryString) {
         try {
            const dataFilters: IDataFilter[] = JSON.parse(
            ...
            const currentUiFilters = dataFilters.map((filter) => {
               ...
               return {
                  ...
                  values: filter.values.map(
                     (value: IDataFilterValueInternal) => {
                        value.selected = true;
            ...
            Log.verbose(
               `[SearchFiltersContainer.getFiltersDeepLink]`,
               `Filters format in the query string is invalid.`
            );
         }
      }
   }
   ...
   private setFiltersDeepLink(submittedFilters: IDataFilter[]) {
      ...
      if (submittedFilters.length > 0) {
         filtersDeepLinkUrl = UrlHelper.addOrReplaceQueryStringParam(
            window.location.href,
            DEEPLINK_QUERYSTRING_PARAM,
            JSON.stringify(submittedFilters)
         );
      } else {
         filtersDeepLinkUrl = UrlHelper.removeQueryStringParam(
            DEEPLINK_QUERYSTRING_PARAM,
            window.location.href
         );
      }
   ...
   private resetFiltersDeepLink() {
      ...
      const filtersDeepLinkUrl = UrlHelper.removeQueryStringParam(
         DEEPLINK_QUERYSTRING_PARAM,
         window.location.href
      );
      window.history.pushState(
   ...
   private _handleQueryStringChange() {
      ...
      window.onpopstate = (ev) => {
         ...
         if (!queryString) {
            this.setState({
               currentUiFilters: this.resetSelectedFilterValues(
                  this.state.currentUiFilters
               ),
               submittedFilters: [],
            });
   ...
   private resetSelectedFilterValues(
      currentUiFilters: IDataFilterInternal[]
   ): IDataFilterInternal[] {
